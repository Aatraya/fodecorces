import os
class submissions:
  def __init__(self,sid=None,uid=None,pid=None,lang=None,code=None,tstamp=None):
...
# changes in validation 
  ...
  def get_code(self, basedirectory):
    submissiondirectory = create_submission_directory(basedirectory, self.problem_id, self.submission_id) #method for saving the submitted code into the appropriate file
    self.directory_path = submissiondirectory
    ...
    file_path = os.path.join(submissiondirectory, filename)
...
def create_submission_directory(basedirectory, problem_id, submission_id): #method for creating a submission folder
  problem_folder = os.path.join(basedirectory, f"problem_{problem_id}")
